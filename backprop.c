...
BPNN *bpnn_internal_create(
    int n_in, int n_hidden, int n_out)
{
  BPNN *newnet;
...
void bpnn_layerforward(
    double *l1, double *l2, double **conn,
    int n1, int n2)
{
  double sum;
  int j, k;
...
void bpnn_adjust_weights(
    double *delta, int ndelta, double *ly, int nly, double **w, double **oldw, double eta, double momentum)
{
  double new_dw;
  int k, j;
  ...
  for (j = 1; j <= ndelta; j++) {
    for (k = 0; k <= nly; k++) {
	  ...
	  if(w[k][j] >= WEIGHT_SATURATE)
		  w[k][j] = WEIGHT_SATURATE-0.0001;

	  else if(w[k][j] <= -1*WEIGHT_SATURATE)
		  w[k][j] = -1*(WEIGHT_SATURATE-0.0001);
    }
  }
}
...
void bpnn_train(
    BPNN *net,
    double eta, double momentum, double *eo, double *eh)
{
  int in, hid, out;
...
BPNN *bpnn_read(char *filename)
{
  ...
  newBPNN = bpnn_internal_create(n1, n2, n3);
  ...
  for (i = 0; i <= n1; i++)
  {
	...
	{
	  fscanf(fd, "%f", &inval);
	  newBPNN->input_weights[i][j]=inval;
    }
  }

  fscanf(fd,"%f",&inval);//reading the value which is written by fprintf(fd,"%f\n",1111111111.11111111111);
  ...
  for (i = 0; i <= n2; i++)
  {
	...
	{
	  fscanf(fd, "%f", &inval);
	  newBPNN->hidden_weights[i][j]=inval;
    }
  }
  ...
  bpnn_zero_weights(newBPNN->input_prev_weights, n1, n2);
  bpnn_zero_weights(newBPNN->hidden_prev_weights, n2, n3);
